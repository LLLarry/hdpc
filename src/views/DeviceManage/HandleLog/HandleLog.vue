        ...
        <el-card class="box-card card_bottom0 cardForm">
          ...
          <el-card class="box-card">
            <el-table
                ...
                >
                <template slot-scope="{row}">
                   <span v-if="row.sort==1">
                       {{row.type == 1 ? '上线' : row.type == 2 ? '下线' : '— —'}}
                   </span>
                   <span v-if="row.sort==2">
                       {{row.type == 1 ? '绑定' : row.type == 2 ? '解绑' : '— —'}}
                   </span>
                   <span v-if="row.sort==3">
                       {{row.type == 1 ? '修改版本号' : row.type == 2 ? '修改收费模板' : '— —'}}
                   </span>
                   <span v-if="row.sort==4">
                       {{row.type == 1 ? '绑定小区' : row.type == 2 ? '解绑小区' : '— —'}}
                   </span>
                </template>
                </el-table-column>
            </el-table>
